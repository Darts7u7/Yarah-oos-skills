...
7. Create Yarah client utility → lib/yarah.ts
...
- Name: `Generate Yarah Token`, trigger: **Post Login**
- Add `jsonwebtoken` as a dependency in the action editor
- Add `YARAH_JWT_SECRET` in the action's **Secrets** tab
...
- Note down **URL** and **Anon Key** from Yarah, then store the CLI value in Auth0 as `YARAH_JWT_SECRET`

## Auth0 Post Login Action

This code runs in Auth0's environment (not your app). The action must sign a JWT with the Yarah secret and attach it as a namespaced custom claim on the ID token.
...
exports.onExecutePostLogin = async (event, api) => {
  const yarahToken = jwt.sign(
    ...
    event.secrets.YARAH_JWT_SECRET,
    { expiresIn: '1h' }
  );

  api.idToken.setCustomClaim('https://yarah.dev/yarah_token', yarahToken);
};
```

## Auth0 v4 SDK — `beforeSessionSaved`

Auth0 v4 SDK **filters out custom claims** from the ID token by default. You **must** configure `beforeSessionSaved` on `Auth0Client` to extract the Yarah token into the session. Without this, `getSession().user` will not contain the token.
...
beforeSessionSaved: async (session, idToken) => {
  if (idToken) {
    ...
    const yarahToken = payload["https://yarah.dev/yarah_token"];
    if (yarahToken) {
      (session.user ??= {})["https://yarah.dev/yarah_token"] = yarahToken;
    }
  }
  return session;
}
...
- Create a utility at `lib/yarah.ts` that calls `auth0.getSession()`, reads the token from `session.user["https://yarah.dev/yarah_token"]`, and passes it as `accessToken` to `createClient`
...
export async function createYarahClient() {
  ...
  const yarahToken = session?.user?.["https://yarah.dev/yarah_token"];
...
| Mistake | Solution |
|---------|----------|
| ❌ Forgetting `beforeSessionSaved` | ✅ Always configure it — without it the Yarah token is silently dropped |
