...
2. Server Primitives & Env     → lib/okx-facilitator.ts, lib/x402.ts, lib/yarah.ts, .env
...
| `NEXT_PUBLIC_YARAH_URL` | Yarah Dashboard → Project Settings | client + server |
| `NEXT_PUBLIC_YARAH_ANON_KEY` | Yarah Dashboard → Project Settings | client |
| `YARAH_SERVICE_KEY` | Yarah Dashboard → Project Settings (server-only) | server |
| `OPENROUTER_API_KEY` | Yarah Dashboard → Model Gateway → Overview | server |
...
```bash
npx -y @yarahdev/cli db import migrations/db_init.sql
```

**✓ Verify**

```bash
npx -y @yarahdev/cli db query "select count(*) from x402_payments" --json
# → rowCount: 1, count: "0"

npx -y @yarahdev/cli db query "select pattern, enabled from realtime.channels where pattern = 'x402_payments'" --json
# → enabled: true

npx -y @yarahdev/cli db query "select tgname from pg_trigger where tgrelid = 'x402_payments'::regclass and not tgisinternal" --json
...
```bash
npm install @yarahdev/sdk viem openai
```
...
**`src/lib/yarah.ts`** — one file exports both clients so you don't instantiate the SDK twice.
...
export function createServiceClient() {
  return createClient({
    ...
    anonKey: process.env.YARAH_SERVICE_KEY!, // service key goes through the anonKey slot (Bearer token)
  });
}
...
# Yarah (dashboard → Project Settings → API Keys)
...
```bash
node --env-file=.env -e "['OKX_API_KEY','OKX_SECRET_KEY','OKX_PASSPHRASE','PAYMENT_RECIPIENT','NEXT_PUBLIC_YARAH_URL','NEXT_PUBLIC_YARAH_ANON_KEY','YARAH_SERVICE_KEY','OPENROUTER_API_KEY'].forEach(k => console.log(k, process.env[k] ? 'ok' : 'MISSING'))"
# → every row 'ok'
```
...
import { createServiceClient } from "@/lib/yarah";
import {
...
export async function POST(req: NextRequest) {
  ...
  const yarah = createServiceClient();
  const { error: insertError } = await yarah.database.from("x402_payments").insert([{
...
export async function GET() {
  ...
  const { data: payments, error } = await yarah.database
    .from("x402_payments")
    .select()
  ...
  const { data: allPayments, error: statsError } = await yarah.database
    .from("x402_payments")
    .select("amount");
...
```typescript
// src/lib/yarah-browser.ts
"use client";
import { createBrowserClient } from "@/lib/yarah";
import type { YarahClient } from "@yarahdev/sdk";

let cached: YarahClient | null = null;
export function getBrowserClient(): YarahClient {
...
import { getBrowserClient } from "@/lib/yarah-browser";
...
export function usePayments() {
  ...
  useEffect(() => {
    ...
    yarah.realtime.on("INSERT_x402_payments", onInsert);

    yarah.realtime.connect()
      .then(() => yarah.realtime.subscribe("x402_payments"))
      .catch(console.error);

    return () => {
      yarah.realtime.off("INSERT_x402_payments", onInsert);
      yarah.realtime.unsubscribe("x402_payments");
    };
  }, [yarah]);
