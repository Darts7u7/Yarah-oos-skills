# Yarah + Better Auth Integration Guide

Better Auth is the only supported auth provider that **runs inside your own Postgres database** — there is no third-party SaaS in the loop. You point Better Auth at Yarah's Postgres via a connection string, it creates `user` / `session` / `account` / `verification` tables in a dedicated `better_auth` schema (hidden from PostgREST by construction — Yarah exposes only `public`), and a small bridge route on your app server signs an HS256 JWT for the Yarah HTTP API. Better Auth's `id` column is a string (not UUID), the same convention every other third-party integration here uses for `user_id`.

Three schemas, three owners — the integration boundary is structural, not procedural:

```
auth.*          ← Yarah platform internals (project admins, OAuth) — untouched
...
The auth/bridge primitives are framework-agnostic — `lib/auth.ts`, the schema setup, RLS policies, plugins, and the `useYarahClient` hook are identical across both. Only the route-handler shape and a few env-var prefixes differ.

## Recommended Workflow

For Next.js apps, the Yarah CLI scaffolds every file in this guide in one command:

```bash
npx -y @yarahdev/cli link --auth better-auth   # or create --auth better-auth for a fresh dir
npm install
...
> **Integrating manually** (no CLI, or non-Next stack)? Sequence: (1) `npx -y @yarahdev/cli create` or `link`, (2) `npx -y @yarahdev/cli secrets get JWT_SECRET`, (3) install deps and fill `.env.local`, (4) write `lib/auth.ts` with `search_path` set to `better_auth, public` (so BA's tables go in the dedicated schema), (5) `CREATE SCHEMA better_auth`, (6) `npx -y @better-auth/cli migrate`, (7) BA route handler, (8) bridge route, (9) `requesting_user_id()` + RLS + `notes` table FK'd to `better_auth.user(id)`, (10) `useYarahClient` (or server-side `createYarahClient`), (11) feature pages. Each numbered step has its own section below.

Starting point: `npx -y @yarahdev/cli link --auth better-auth` (or `create`) scaffolds a working Next 15 + BA project. The `--auth` flag is canonical; the rest of this guide explains the pieces it generates. For Vite/React or other non-Next stacks, see [Vite / React-only setups](#vite--react-only-setups) below — the proxy config and bridge route map directly.
...
- Get the JWT secret: `npx -y @yarahdev/cli secrets get JWT_SECRET` — used to sign the bridge JWT
- Get the Postgres connection string for Better Auth's pool — for self-hosted Yarah, the docker-compose exposes `POSTGRES_PORT` (default `5432`) with the project's `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_DB`
- Note **Base URL** and **Anon Key** from the Yarah dashboard
...
export const auth = betterAuth({
  ...
  secret: requireEnv('BETTER_AUTH_SECRET'),         // Better Auth's own session secret — different from Yarah's JWT_SECRET
...
> **Why this is enough.** PostgREST is configured to expose only the `public` schema (`PGRST_DB_SCHEMAS=public`). Anything in `better_auth` is invisible to the data API, so anon and authenticated SDK calls return `404 relation "public.user" does not exist` instead of leaking emails. No REVOKE step. The Yarah dashboard reaches `better_auth.*` through its admin route (postgres superuser pool, role-independent), so Studio inspection works.
...
This is where the integration lives. Better Auth's own `jwt()` plugin issues asymmetric tokens (EdDSA/ES256/RS256) which Yarah's PostgREST cannot verify — it expects HS256 signed with the Yarah JWT secret. So we re-sign:

```ts
// app/api/yarah-token/route.ts
import { auth } from '@/lib/auth';
...
export async function GET() {
  ...
  // Mint the smallest claim set Yarah needs. Don't add email or other PII —
...
import { AuthChangeEvent, createClient, type YarahClient } from '@yarahdev/sdk';
...
Better Auth sign-out doesn't clear the Yarah SDK's in-memory token. Pattern A handles this automatically via the `useEffect` cleanup; if you sign out outside of React, do it explicitly with `client.setAccessToken(null)` (clears both HTTP and realtime in one call):
...
Better Auth user IDs are **strings** (e.g. `f5kGYiUXDPEJqRDQ4jgtNTopIzpj5MgK`), not UUIDs. Use `TEXT` for any FK referencing them, and FK to `better_auth.user(id)` — never to `auth.users(id)` (Yarah's separate native auth table, UUID id, irrelevant to BA).
...
Prefer running this through the Yarah CLI (`npx -y @yarahdev/cli db migrations new ... && npx -y @yarahdev/cli db migrations up`) — the CLI emits the `NOTIFY` automatically. Raw `psql` works but you must send the notify yourself or PostgREST returns `404 {}` until next reload.

## Realtime (optional)

If you use `client.realtime`, two extra one-time setup steps are needed because Better Auth IDs are strings (not UUIDs) and Yarah realtime currently requires both manual channel registration and a column-type fix.
...
Better Auth invokes `sendVerificationEmail` and `sendResetPassword` callbacks on signup and reset flows. Wire those callbacks to Yarah's `client.emails.send()` so all transactional mail goes through one provider.
...
// secret that the bridge route uses. Reusing YARAH_JWT_SECRET keeps the
// trust boundary minimal.
function yarahServerClient() {
  const token = jwt.sign(
...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    ...
    sendResetPassword: async ({ user, url }) => {
      const yarah = yarahServerClient();
      const { error } = await yarah.emails.send({
        to: user.email,
...
### Where Yarah actually sends from

`client.emails.send` calls `POST /api/email/send-raw`. Yarah resolves the provider per-call:

1. **SMTP** — if you set SMTP credentials via `PUT /api/auth/smtp-config` (admin token), every send goes through your SMTP server.
2. **Cloud fallback** — if no SMTP is configured, Yarah tries its managed cloud relay. Requires `PROJECT_ID` (set automatically on cloud-hosted projects; missing on self-hosted).
...
```ts
// app/api/yarah-token/route.ts (delta)
const token = jwt.sign(
  {
    sub: session.user.id,
    role: 'authenticated',
...
`vite.config.ts` proxies `/api` to the BA server. To the browser everything is on `:5173`, so the BA cookie is auto-attached to `/api/yarah-token` with no CORS dance.
...
app.get('/api/yarah-token', async (c) => {
...
Express is the same shape — `app.all('/api/auth/*', toNodeHandler(auth))` (use `better-auth/node` instead of `better-auth/next-js`), then a regular `app.get('/api/yarah-token', ...)` handler. The JWT signing block is identical.

### Env vars in Vite

Vite exposes `import.meta.env.VITE_*` instead of `process.env.NEXT_PUBLIC_*`. Anything server-only (`DATABASE_URL`, `BETTER_AUTH_SECRET`, `YARAH_JWT_SECRET`) reads from `process.env` in the BA server process — same as Next.

```bash
# .env.local  (Vite project)
VITE_BETTER_AUTH_URL=http://localhost:5173      # the SPA origin (or BA origin if cross-origin)
VITE_YARAH_BASE_URL=http://localhost:7130
VITE_YARAH_ANON_KEY=<from Yarah dashboard>
```

`lib/auth-client.ts` and `lib/yarah.ts` then read `import.meta.env.VITE_BETTER_AUTH_URL` etc. The Pattern A hook code shown earlier is otherwise identical.
...
4. **Client fetch**: `fetch('/api/yarah-token', { credentials: 'include' })` — without it the BA cookie isn't sent.
...
|----------|--------|-----------|
| `DATABASE_URL` | Yarah Postgres connection string | Server (BA's `Pool`) |
...
| `YARAH_JWT_SECRET` | `npx -y @yarahdev/cli secrets get JWT_SECRET` | Server (bridge route HS256 signing) |
...
| `NEXT_PUBLIC_YARAH_BASE_URL` / `VITE_YARAH_BASE_URL` | Yarah dashboard | Browser (SDK baseUrl) |
| `NEXT_PUBLIC_YARAH_ANON_KEY` / `VITE_YARAH_ANON_KEY` | Yarah dashboard | Browser (SDK anonKey) |

## Common Mistakes

| Mistake | Solution |
|---------|----------|
| ❌ Forgetting `NOTIFY pgrst, 'reload schema'` after raw psql DDL | ✅ PostgREST returns `404 {}` until reloaded. Use the Yarah CLI for migrations and the notify happens automatically. |
| ❌ Using Better Auth's `jwt()` plugin directly with Yarah | ✅ It issues asymmetric (EdDSA/ES256/RS256) tokens; Yarah's PostgREST verifies HS256. Use the bridge route instead. |
| ❌ Using `auth.uid()` for RLS policies | ✅ Use `requesting_user_id()` — Better Auth IDs are strings, not UUIDs. |
| ❌ FK'ing to `auth.users(id)` (or `public.user(id)`) | ✅ FK to `better_auth.user(id)` — that's where Better Auth puts its tables. `auth.users` is Yarah's native auth (UUID id, irrelevant here); `public.user` no longer exists post-migrate. |
...
| ❌ Re-using `BETTER_AUTH_SECRET` as the Yarah JWT secret | ✅ They are independent. `BETTER_AUTH_SECRET` is for Better Auth's session cookies; `YARAH_JWT_SECRET` is the HS256 key for the bridge JWT. |
...
| ❌ Realtime publish silently fails for authenticated users (`UNAUTHORIZED`) | ✅ `realtime.messages.sender_id` is `uuid` in core Yarah; Better Auth IDs are strings. One-time fix: `ALTER TABLE realtime.messages ALTER COLUMN sender_id TYPE text;` |
