...
# Yarah + Stytch Integration Guide

Stytch handles authentication via email magic links on the client side. On the server, the Stytch Node SDK validates the session cookie, retrieves the user ID, and signs a JWT with Yarah's secret. The token is passed to Yarah as `accessToken` (deprecated alias: `edgeFunctionToken`).
...
7. Create Yarah client utility  → lib/yarah.ts (server-side session validation + JWT signing)
...
- Required claims: `sub` (from `session.user_id`), `role: "authenticated"`, `aud: "yarah-api"`
