...
# Yarah + Clerk Integration Guide (Next.js)

Clerk signs tokens with Yarah's JWT secret directly via a **JWT Template** — no server-side signing needed. The app calls `getToken({ template: 'yarah' })` and forwards the token to the Yarah client via `client.setAccessToken()`.
...
6. Initialize Yarah client      → createClient + setAccessToken with Clerk token (refresh on interval)
7. Set up Yarah database        → requesting_user_id() + table + RLS
8. Build features                  → CRUD pages using Yarah client
```
...
- Signing key: the Yarah JWT Secret
- Claims: `{ "role": "authenticated", "aud": "yarah-api" }`
...
- Note down **URL** and **Anon Key** from Yarah, then use the CLI output as the signing key in Clerk
...
- In a `useEffect` keyed on `isSignedIn` and Clerk `userId`, call `getToken({ template: 'yarah' })` and pipe the result into `client.setAccessToken(token, event)` (see the next bullet for choosing `event`)
...
- The template name `'yarah'` must match the Clerk dashboard exactly
- `@yarahdev/sdk`'s `accessToken` config field (deprecated alias: `edgeFunctionToken`) is a **static string**, not a function — it cannot auto-refresh on its own, which is why we use `client.setAccessToken()` imperatively
...
export function useYarahClient(): { client: YarahClient; isReady: boolean } {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const token = await getToken({ template: 'yarah' });
        if (cancelled) return;
        if (!token) {
          client.setAccessToken(null);
          currentUserIdRef.current = null;
        ...
        currentUserIdRef.current = null;
        setIsReady(false);
        console.error('Failed to refresh Clerk token for Yarah client', err);
      }
    };
...
| `NEXT_PUBLIC_YARAH_BASE_URL` | Yarah Dashboard | Exposed to the browser |
| `NEXT_PUBLIC_YARAH_ANON_KEY` | Yarah Dashboard | Exposed to the browser |
...
| ❌ Forgetting `'use client'` on `lib/yarah.ts` | ✅ The hook uses React + Clerk hooks; the file must be a client module |
