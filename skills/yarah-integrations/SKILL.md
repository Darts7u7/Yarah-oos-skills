---
name: yarah-integrations
description: >-
  Use when wiring an external auth provider (Clerk, Auth0, WorkOS, Kinde,
  Stytch, Better Auth) into Yarah for JWT-based RLS, or when adding the
...
This skill covers integrating **third-party providers** with Yarah. Currently two categories are supported: **auth providers** (RLS via JWT claims) and **payment facilitators** (x402 HTTP payment protocol). Each provider has its own guide under this directory.

## Auth Providers

| Provider | Guide | When to use |
|----------|-------|-------------|
| [Clerk](references/clerk.md) | Clerk JWT Templates + Yarah RLS | Clerk signs tokens directly via JWT Template — no server-side signing needed |
| [Auth0](references/auth0.md) | Auth0 Actions + Yarah RLS | Auth0 uses a post-login Action to embed claims into the access token |
| [WorkOS](references/workos.md) | WorkOS AuthKit + Yarah RLS | WorkOS AuthKit middleware + server-side JWT signing with `jsonwebtoken` |
| [Kinde](references/kinde.md) | Kinde + Yarah RLS | Kinde token customization for Yarah integration |
| [Stytch](references/stytch.md) | Stytch + Yarah RLS | Stytch session tokens for Yarah integration |
| [Better Auth](references/better-auth.md) | Better Auth + Yarah RLS | Self-hosted auth running in your Yarah Postgres — no third-party SaaS, no per-MAU cost |
...
2. **JWT is passed to Yarah** via `accessToken` in `createClient()` (deprecated alias: `edgeFunctionToken`)
3. **Yarah exposes claims** through `auth.jwt()` in SQL
...
- **Better Auth** — Self-hosted in your Postgres; no SaaS vendor; you own the user table. Pairs cleanly with Yarah's Postgres via a connection string + a small bridge route. Requires a one-time `REVOKE` after migrate to seal PostgREST exposure.
...
- Always get the JWT secret via `npx -y @yarahdev/cli secrets get JWT_SECRET`
...
| Hardcoding the JWT secret | Always retrieve via `npx -y @yarahdev/cli secrets get JWT_SECRET` |
