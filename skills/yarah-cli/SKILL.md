...
Use this skill whenever someone needs a backend, or when managing Yarah backend and cloud infrastructure with the Yarah CLI. For application code that calls Yarah from a frontend, backend, or edge function, use the `yarah` app-integration skill instead.

## Core Rules

- Always run the CLI through `npx -y @yarahdev/cli <command>`. Keep npx's `-y`: without it, npx asks "Ok to proceed?" before installing the package and blocks forever in a TTY-attached agent shell. Do not install or call a global `yarah` binary.
...
- At the start of a non-trivial task on a linked project, run `npx -y @yarahdev/cli memory list` (cheap, no AI call) and recall any title relevant to the task before designing or debugging. Record decisions and the gotchas you hit with `memory remember` at the moment they happen. See `references/memory.md`.
- When you hit a hurdle that is Yarah's fault — something that should work but doesn't, a capability you needed but isn't supported, instructions (docs/skill) that reality contradicts, or needless friction — report it with `npx -y @yarahdev/cli feedback` (see Feedback), then continue the user's task with a workaround. Never file feedback for problems in the user's own app code.
...
| `-y`, `--yes` | Auto-accept Y/N confirmation prompts such as delete or overwrite prompts. Does not skip value-collection prompts; use `--json` for that. Separate from npx's own `-y`, so both appear together: `npx -y @yarahdev/cli link --project-id <id> -y`. |
...
| Variable                | Use                                |
| ----------------------- | ---------------------------------- |
| `YARAH_ACCESS_TOKEN` | Override stored access token       |
| `YARAH_PROJECT_ID`   | Override linked project ID         |
| `YARAH_EMAIL`        | Email for non-interactive login    |
| `YARAH_PASSWORD`     | Password for non-interactive login |

## Connection Setup

If a task needs project access and the connection state is unknown, start with `npx -y @yarahdev/cli current`. Use `npx -y @yarahdev/cli whoami` when the authenticated identity matters or when `current` reports that the CLI is not authenticated.

If not authenticated, run `npx -y @yarahdev/cli login` (opens a browser). For headless / agent / CI contexts with no browser, authenticate non-interactively with a user API key: `npx -y @yarahdev/cli login --user-api-key "$YARAH_USER_API_KEY"` (the user creates the key in the dashboard under Profile → API Keys). In sandboxes where the user has a browser but it cannot reach the CLI's local callback (e.g. the ChatGPT app), use device login as two steps: `timeout 15 npx -y @yarahdev/cli login --device --json 2>&1 || true` to capture the verification link + code, relay them to the user, then rerun `npx -y @yarahdev/cli login --device --json` to resume the same code and complete once they click Authorize — see `references/login.md`. If the sandbox reports that `api.yarah.dev` is not an allowed network domain, ask the user to add it to the workspace's allowed network domains, then retry. If no project is linked, use `npx -y @yarahdev/cli link` for an existing project or `npx -y @yarahdev/cli create` when the user asked for a new backend. In workflows that are already prelinked or preconfigured, such as CI, local test projects, automation, or explicit user-provided project context, use that project context directly. A cloud project is the default throughout; only when the user explicitly asks for a backend running in Docker on their own machine, see `references/local.md` — never as a fallback when login or `create` is inconvenient.
...
- Prefer `npx -y @yarahdev/cli db migrations new <name>` plus a migration SQL file for schema, grants, indexes, triggers, functions, and RLS policy changes.
- Apply migrations with `npx -y @yarahdev/cli db migrations up --all`.
...
- Use `npx -y @yarahdev/cli db query <sql>` for targeted inspection and small corrective row/data SQL only when a migration is not appropriate.
- Use `npx -y @yarahdev/cli db rpc <fn> [--data <json>]` to call database functions through the backend.
...
- Do not create custom schemas or write to Yarah-managed/system schemas such as `auth`, `storage`, `realtime`, `payments`, `graphql`, `extensions`, `pg_catalog`, `information_schema`, or `system`, unless you are working on that specific feature module and its docs explicitly allow the operation.
...
- `npx -y @yarahdev/cli create` - create a new project. Use `--json` with required flags for non-interactive agent runs. See `references/create.md`.
- `npx -y @yarahdev/cli link` - link the current directory to an existing project.
- `npx -y @yarahdev/cli link --api-base-url <url> --api-key <admin key>` - link a self-hosted (OSS) backend directly by its URL and admin API key; no platform login required.
- `npx -y @yarahdev/cli current` - show current linked project.
- `npx -y @yarahdev/cli metadata --json` - inspect backend metadata when discovery is needed.

Project lifecycle (operates on the linked project unless `--project <id>` is given):

- `npx -y @yarahdev/cli projects get [--project <id>]` - show a project's current status, in-flight `operation_status`, region, instance type, and version. Use this to poll after an async operation (restore, version update, instance resize) until `operation_status` clears.
- `npx -y @yarahdev/cli projects update [--name <name>] [--domain <domain>] [--storage-size <gib>] [--project <id>]` - rename or change project settings.
- `npx -y @yarahdev/cli projects restore [--project <id>]` - bring a paused project back online. Only paused projects can be restored.
- `npx -y @yarahdev/cli projects update-version [--wait] [--project <id>]` - update the backend to the latest Yarah version (resolved automatically; no-op if already current). Causes a brief restart. Add `--wait` to block until it finishes instead of returning while queued.
- `npx -y @yarahdev/cli projects upgrade-instance <type> [--project <id>]` - change the instance class. Valid: `nano`, `micro`, `small`, `medium`, `large`, `xl` (`xl` is the ceiling). Restarts the project and changes the bill.
- `npx -y @yarahdev/cli projects delete --project <id>` - permanently delete a project and all of its resources. `--project` is required (it will not default to the linked project). Irreversible — confirm the exact project id with the user first; this is a guarded, human-in-the-loop operation, so do not auto-bypass the confirmation.
- `npx -y @yarahdev/cli projects transfer <targetOrgId> --project <id>` - move a project to another organization (billing and access move with it). `--project` is required (it will not default to the linked project). Guarded, human-in-the-loop — confirm the source project and target org first.

Configuration:

- Use `npx -y @yarahdev/cli config export`, `config plan`, and `config apply` for supported `yarah.toml` knobs.
...
Org-scoped commands resolve the organization in this order: `--org-id` flag, `YARAH_ORG_ID`, the linked project's org, the configured default org, then a prompt (or single-org auto-select). Pass `--org-id <id>` to act on a specific org.

- `npx -y @yarahdev/cli orgs list` - list organizations you belong to.
- `npx -y @yarahdev/cli orgs create <name> [--type personal|team|company]` - create an organization (default type `team`).
- `npx -y @yarahdev/cli orgs update [--name <name>] [--type <type>] [--org-id <id>]` - rename or change an organization's type.
- `npx -y @yarahdev/cli orgs members list [--org-id <id>]` - list members and pending invitations.
- `npx -y @yarahdev/cli orgs members invite <email> [--role administrator|developer] [--org-id <id>]` - invite a member (default role `developer`).
- `npx -y @yarahdev/cli orgs members role <memberId> <role> [--org-id <id>]` - change a member's role (`administrator` or `developer`).
- `npx -y @yarahdev/cli orgs members remove <memberId> [--org-id <id>]` - remove a member. Confirm intent first.
- `npx -y @yarahdev/cli orgs leave --org-id <id>` - leave an organization. `--org-id` is required (it will not default to the linked org). You lose access to all of its projects and must be re-invited to return. The backend refuses if you are the last administrator — transfer the admin role first. Guarded, human-in-the-loop — confirm intent first.
- `npx -y @yarahdev/cli orgs delete --org-id <id>` - permanently delete an organization. `--org-id` is required (it will not default to the linked org). Owner only. This cascades: every project in the org (databases, storage, all resources) is permanently deleted and the subscription is canceled — the CLI lists the affected projects and warns when the currently linked project is one of them. Irreversible; confirm the exact org id with the user first and do not auto-bypass the confirmation.

## Billing and Usage

Inspect the organization's plan/consumption and manage its subscription. Org resolution matches the Organizations section.

- `npx -y @yarahdev/cli billing status [--org-id <id>]` - show the current subscription/plan and period.
- `npx -y @yarahdev/cli billing credits [--org-id <id>]` - show the credit balance and recent credit transactions.
- `npx -y @yarahdev/cli billing history [--org-id <id>]` - list past payments / invoices.
- `npx -y @yarahdev/cli billing cycles [--org-id <id>]` - show the current and previous billing-cycle windows.
- `npx -y @yarahdev/cli usage [--org-id <id>]` - show consumption for the current billing period (summary plus per-project breakdown: database, storage, egress, etc.).
- `npx -y @yarahdev/cli billing upgrade <plan> [--org-id <id>]` - start a Stripe checkout to change the plan (`free | starter | pro | team | enterprise`). Opens the hosted checkout URL in the browser and also prints it. With `--json` it prints a JSON object (`{ checkoutUrl, sessionId }`) and does not open a browser — use this in headless/CI. No charge happens until the user completes checkout; the backend validates the plan and admin permission.
- `npx -y @yarahdev/cli billing manage [--org-id <id>]` - open the Stripe customer portal to manage the subscription, payment method, or cancellation. Opens the portal URL in the browser and also prints it. With `--json` it prints a JSON object (`{ portalUrl }`) and does not open a browser — use this in headless/CI.
...
- `npx -y @yarahdev/cli backups list [--project <id>]` - list backups.
- `npx -y @yarahdev/cli backups latest [--project <id>]` - show the most recent backup. Cloud prints the latest dump file with a presigned download URL; self-hosted prints the newest backup record (no download URL).
- `npx -y @yarahdev/cli backups create [--name <name>] [--wait] [--project <id>]` - create a backup. `--name` is optional; when provided it must be 1–64 chars. `--wait` blocks until it finishes instead of returning while queued.
- `npx -y @yarahdev/cli backups rename <backupId> <name> [--project <id>]` - rename a backup (pass `""` to clear the name).
- `npx -y @yarahdev/cli backups delete <backupId> [--project <id>]` - delete a backup. Confirm intent first.
- `npx -y @yarahdev/cli backups restore <backupId> [--project <id>]` - restore the project from a backup. Confirm intent first. Cloud: OVERWRITES the project's current database and storage — all data written since the backup is lost. Self-hosted: database-only `pg_restore --clean` — data in backed-up tables is rewound, but tables created after the backup are NOT dropped and storage is untouched.

## Storage

- `npx -y @yarahdev/cli storage buckets` - list buckets.
- `npx -y @yarahdev/cli storage create-bucket <name> [--private]` - create a bucket.
- `npx -y @yarahdev/cli storage delete-bucket <name>` - delete a bucket and all objects. Confirm destructive intent first.
- `npx -y @yarahdev/cli storage list-objects <bucket> [--prefix] [--search] [--limit] [--sort]` - inspect objects.
- `npx -y @yarahdev/cli storage upload <file> --bucket <name> [--key <objectKey>]` - upload an object.
- `npx -y @yarahdev/cli storage download <objectKey> --bucket <name> [--output <path>]` - download an object.
- `npx -y @yarahdev/cli storage s3-keys list` - list S3-compatible access keys (secret values are never shown).
- `npx -y @yarahdev/cli storage s3-keys create [--description <text>]` - create an S3 access key. The secret access key is shown ONCE on creation — capture it immediately.
- `npx -y @yarahdev/cli storage s3-keys delete <id>` - delete an S3 access key. Tools using it stop working. Confirm intent first.
...
- `npx -y @yarahdev/cli functions list` - list deployed functions.
- `npx -y @yarahdev/cli functions code <slug>` - view function source.
- `npx -y @yarahdev/cli functions deploy <slug> --file <path>` - deploy or update. See `references/functions-deploy.md`.
- `npx -y @yarahdev/cli functions invoke <slug> [--data <json>] [--method GET|POST]` - invoke a function.
- `npx -y @yarahdev/cli functions delete <slug>` - delete a function. Confirm destructive intent first.

## AI Gateway

- `npx -y @yarahdev/cli ai setup` fetches the linked project's active OpenRouter key and writes `OPENROUTER_API_KEY` to a local server-side env file.
- `npx -y @yarahdev/cli ai overview` shows Model Gateway key usage: total spend, limit, remaining credit, daily/weekly/monthly spend, and per-model activity when observability is available. Figures are USD credits. Use it to answer "how much AI credit is left / being used".
...
- `npx -y @yarahdev/cli memory list` - cheap title index (no AI call). Run at the start of a non-trivial task; recall any title relevant to the task.
- `npx -y @yarahdev/cli memory recall "<query>" [--scope] [--limit] [--threshold]` - semantic + keyword recall.
- `npx -y @yarahdev/cli memory remember "<content>" [--kind] [--title] [--scope] [--source]` - store one atomic memory. Record decisions and gotchas at the moment they happen, not at session end. `--kind` accepts only `fact`, `decision`, `preference`, or `reference` - store gotchas as `fact` (or `decision` when recording a choice).
- `npx -y @yarahdev/cli memory remember --file <path>` - extract durable memories from a transcript or notes file.
...
Runtime checkout, subscriptions, customer portal flows, and app code belong in the `yarah` app-integration skill.
...
- Use `npx -y @yarahdev/cli deployments deploy <dir>` for frontend source directories. Do not deploy generated output directories unless the deployment reference explicitly calls for it.
- See `references/deployments/deploy.md`.

Custom domains:

- Use `npx -y @yarahdev/cli domains ...` for custom domains, Cloudflare Registrar, DNS sync, and SSL verification.
- See `references/deployments/domains.md`.

Backend compute services:

- Use `npx -y @yarahdev/cli compute ...`; do not manage Yarah compute services directly with the user's own `flyctl` account.
...
- `npx -y @yarahdev/cli secrets list [--all]` - list secret keys without values.
- `npx -y @yarahdev/cli secrets get <key>` - retrieve a secret value only when necessary.
- `npx -y @yarahdev/cli secrets add <key> <value> [--reserved] [--expires <ISO date>]` - create a secret.
- `npx -y @yarahdev/cli secrets update <key> [--value] [--active] [--reserved] [--expires]` - update a secret.
- `npx -y @yarahdev/cli secrets delete <key>` - soft-delete a secret. Confirm intent first.
- `npx -y @yarahdev/cli secrets rotate <api-key|anon-key> [--grace-hours <n>]` - rotate the project API key or anon key. The new key is printed ONCE — capture it. The old key keeps working during the grace period (server default if `--grace-hours` is omitted); update all consumers before it expires.

## Schedules

- `npx -y @yarahdev/cli schedules list/get/create/update/delete/logs`.
...
- Headers can reference Yarah secrets with `${{secrets.KEY_NAME}}`.
...
- `npx -y @yarahdev/cli branch create <name> [--mode full|schema-only] [--no-switch]`
- `npx -y @yarahdev/cli branch list`
- `npx -y @yarahdev/cli branch switch <name>` or `--parent`
- `npx -y @yarahdev/cli branch merge <name> [--dry-run] [--save-sql <path>]`
- `npx -y @yarahdev/cli branch reset <name>`
- `npx -y @yarahdev/cli branch delete <name>`
...
- `npx -y @yarahdev/cli diagnose` - full health report.
- `npx -y @yarahdev/cli diagnose --ai "<issue description>"` - ask the Yarah debug agent to diagnose a concrete backend issue.
- `npx -y @yarahdev/cli diagnose metrics [--range 1h|6h|24h|7d]` - EC2 metrics.
- `npx -y @yarahdev/cli diagnose advisor [--severity critical|warning|info] [--category security|performance|health]` - advisor issues. The Rule column is the id that `advisor suppress` takes.
- `npx -y @yarahdev/cli diagnose db [--check <checks>]` - database health checks.
- `npx -y @yarahdev/cli diagnose logs [--source <name>] [--limit <n>]` - aggregate error logs.
- `npx -y @yarahdev/cli logs <source> [--limit <n>]` - source-specific backend logs.

Typical log sources include `function.logs`, `function-deploy.logs`, `postgres.logs`, `postgrest.logs`, and `yarah.logs`. See `references/diagnostics.md` for common debugging scenarios and source selection.
...
- `npx -y @yarahdev/cli advisor scan` - trigger a scan now instead of waiting for the schedule. Use it to re-check immediately after fixing a finding. The scan runs asynchronously (typically well under a minute) — poll `diagnose advisor --json` until `scan.status` is `completed` and `scan.scanId` equals the id that `advisor scan` returned, then read the results.
- `npx -y @yarahdev/cli advisor suppressions` - list suppressed findings.
- `npx -y @yarahdev/cli advisor suppress <ruleId> [--object <affectedObject>] --reason <reason> [--note <note>]` - dismiss a finding with a recorded reason. With `--object` (the finding's Affected Object, verbatim) only that instance is suppressed; without it the whole rule is. `--reason` is one of `false_positive | accepted_risk | wont_fix | other`; `--note` is required for `other`. A suppression takes effect from the next scan (run `advisor scan` to see it applied). Only suppress findings the user has judged — never suppress to make a report look clean.
- `npx -y @yarahdev/cli advisor unsuppress <suppressionId>` - remove a suppression so the finding reappears on the next scan.

## Feedback

When any part of the Yarah toolkit misbehaves — the backend platform, an SDK, the CLI, an agent skill, or the docs — report it to the Yarah team, then continue the task with a workaround. Only report Yarah-side issues, never problems in the user's own app code.
...
npx -y @yarahdev/cli feedback --json \
  ...
  --area db \
  --command "yarah db policies create --table Users ..." \
...
Keep `--detail` concise and Yarah-focused: what happened, what you expected, minimal repro. Do not paste user app data — the CLI locally redacts common patterns (emails, known credential/key formats, secret assignments, public IPv4 addresses, home-directory usernames) and truncates long fields, but redaction is pattern-based: a safety net, not a license. No login required — works logged out and in OSS setups; project/org context is attached automatically when a cloud project is linked. Returns a feedback id on success (duplicate reports fold into the existing one and return its id).

## Documentation

- `npx -y @yarahdev/cli docs` - list documentation topics.
- `npx -y @yarahdev/cli docs instructions` - setup guide.
- `npx -y @yarahdev/cli docs <feature> <language>` - feature docs for `db`, `storage`, `functions`, `auth`, `ai`, or `realtime` in `typescript`, `swift`, `kotlin`, or `rest-api`.

For application code with Yarah or `@yarahdev/sdk`, use the `yarah` app-integration skill and use `docs` only as official feature reference.

## PostHog

- `npx -y @yarahdev/cli posthog setup` ensures the dashboard has a PostHog connection, then prints the official PostHog wizard command plus the connected project's public `phc_` API key and host.
...
- `npx -y @yarahdev/cli webscraper apify connect` — one-time OAuth connect; stores a refreshable token in Yarah.
- `npx -y @yarahdev/cli webscraper apify login` — auth bridge: fetches the Yarah-managed token, runs `apify login --token`, and installs Apify's official agent skills. Never run plain `apify login` (browser OAuth). On any Apify `401` / "not logged in", re-run `login`.
...
```bash
YARAH_EMAIL=$EMAIL YARAH_PASSWORD=$PASSWORD npx -y @yarahdev/cli login --email -y
npx -y @yarahdev/cli link --project-id $PROJECT_ID --org-id $ORG_ID -y
npx -y @yarahdev/cli db query "SELECT 1 AS ok" --json
```

## Project Configuration File

After `create` or `link`, `.yarah/project.json` contains the linked project ID, app key, region, API key, and backend URL.

- Never commit `.yarah/project.json` or share it publicly.
- Do not edit it manually. Use `npx -y @yarahdev/cli link` or branch commands to switch projects.
