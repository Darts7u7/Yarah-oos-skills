# npx -y @yarahdev/cli payments razorpay

Use this reference when configuring or inspecting Razorpay payment infrastructure. For app order/subscription code, load `skills/yarah/payments/razorpay.md`.
...
```bash
npx -y @yarahdev/cli payments razorpay config set --environment test --key-id rzp_test_xxx --key-secret xxx
npx -y @yarahdev/cli payments razorpay status
```
...
```bash
npx -y @yarahdev/cli payments razorpay sync --environment test
```
...
Razorpay does not support Yarah-style automatic webhook registration with only API keys. Configure webhooks manually in the Razorpay Dashboard.

From the Yarah dashboard (Dashboard -> Payments -> Settings -> Webhooks), copy:
...
```bash
npx -y @yarahdev/cli payments razorpay catalog --environment test
npx -y @yarahdev/cli payments razorpay items list --environment test
npx -y @yarahdev/cli payments razorpay items create --environment test --name "Pro Plan" --amount 200000 --currency inr
npx -y @yarahdev/cli payments razorpay items update item_123 --environment test --active false
npx -y @yarahdev/cli payments razorpay plans list --environment test
npx -y @yarahdev/cli payments razorpay plans create --environment test --period monthly --interval 1 --item-name "Pro Plan" --item-amount 200000 --item-currency inr
```
...
```bash
npx -y @yarahdev/cli payments razorpay customers --environment test
npx -y @yarahdev/cli payments razorpay subscriptions --environment test
npx -y @yarahdev/cli payments razorpay subscriptions --environment test --subject-type team --subject-id team_123
npx -y @yarahdev/cli payments razorpay transactions --environment test
npx -y @yarahdev/cli payments razorpay transactions --environment test --limit 20 --json
```

`--subject-type` and `--subject-id` are app billing subjects passed to Yarah, such as `team:team_123` or `user:user_123`. They are not Razorpay customer, order, payment, plan, or subscription IDs.
