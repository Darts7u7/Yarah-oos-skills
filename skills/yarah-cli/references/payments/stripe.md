# npx -y @yarahdev/cli payments stripe

Use this reference when configuring or inspecting Stripe payment infrastructure. For app checkout code, load `skills/yarah/payments/stripe.md`.
...
```bash
npx -y @yarahdev/cli payments stripe config set --environment test sk_test_xxx
npx -y @yarahdev/cli payments stripe status
```
...
```bash
npx -y @yarahdev/cli payments stripe sync --environment test
```
...
Stripe webhook registration is automated by Yarah when the backend has a public URL:

```bash
npx -y @yarahdev/cli payments stripe webhooks configure --environment test
```

Yarah configures these Stripe events:
...
```bash
npx -y @yarahdev/cli payments stripe catalog --environment test
npx -y @yarahdev/cli payments stripe products list --environment test
npx -y @yarahdev/cli payments stripe products get prod_123 --environment test
npx -y @yarahdev/cli payments stripe products create --environment test --name "Pro Plan"
npx -y @yarahdev/cli payments stripe products update prod_123 --environment test --description "Updated"
npx -y @yarahdev/cli payments stripe products delete prod_123 --environment test -y
npx -y @yarahdev/cli payments stripe prices list --environment test
npx -y @yarahdev/cli payments stripe prices create --environment test --product prod_123 --currency usd --unit-amount 2000
npx -y @yarahdev/cli payments stripe prices create --environment test --product prod_123 --currency usd --unit-amount 2000 --interval month
npx -y @yarahdev/cli payments stripe prices update price_123 --environment test --active false
npx -y @yarahdev/cli payments stripe prices archive price_123 --environment test
```
...
```bash
npx -y @yarahdev/cli payments stripe customers --environment test
npx -y @yarahdev/cli payments stripe subscriptions --environment test
npx -y @yarahdev/cli payments stripe subscriptions --environment test --subject-type team --subject-id team_123
npx -y @yarahdev/cli payments stripe transactions --environment test
npx -y @yarahdev/cli payments stripe transactions --environment test --limit 20 --json
```

`--subject-type` and `--subject-id` are app billing subjects passed to Yarah, such as `team:team_123` or `user:user_123`. They are not Stripe customer, payment, price, or subscription IDs.
...
Stripe gives no ordering guarantee across events: `invoice.paid` can be processed before `checkout.session.completed` creates the `payments.customer_mappings` row. For subscription events, resolve the billing subject from the payload first (`payload -> 'data' -> 'object' -> 'parent' -> 'subscription_details' -> 'metadata' ->> 'yarah_subject_id'` on invoices) and use `payments.customer_mappings` only as a fallback. See the `yarah` app-integration skill's Stripe guide for a complete subscription fulfillment trigger.
