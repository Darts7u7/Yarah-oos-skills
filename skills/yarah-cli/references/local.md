...
Yarah locally", "I want it in Docker", "run it in a container here".
...
- `npx -y @yarahdev/cli local start` - start the stack, wait for health, link this
  directory, seed `.env.local`. Flags: `--storage <local|minio|rustfs>`, `--pull`,
  `--port-app <n>` (and `--port-auth`, `--port-deno`, `--port-postgres`,
  `--port-postgrest`).
- `npx -y @yarahdev/cli local status [--show-keys] [--json]` - health, ports,
  backend version, per-container state. Keys are masked unless `--show-keys`.
- `npx -y @yarahdev/cli local stop [--delete-data] [--unlink]` - stop the stack.
...
Platform commands still reach Yarah Cloud and still need an account:
...
The first start fetches `deploy/setup.sh` from the Yarah repository and runs it
into `.yarah/checkout/`, then runs the compose file that script wrote — the
same one self-hosting uses. The CLI adds one overlay: the telemetry stamp, and
loopback binding for the published ports.

`.yarah/checkout/.env` holds the generated secrets. `local start` writes
`.yarah/.gitignore` covering `checkout/`, so `git add -A` cannot commit them —
...
instance per directory. Deploying Yarah to a server is `deploy/setup.sh`
directly — see the Yarah repository's deployment docs. Do not point a user
