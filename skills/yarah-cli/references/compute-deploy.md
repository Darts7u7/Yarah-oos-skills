# npx -y @yarahdev/cli compute deploy — deploy a backend container

> 🔧 **DO NOT call `flyctl` directly to manage Yarah compute services.**
> Yarah runs containers on Fly.io under the hood, but the Fly account, org,
> IPs, and machine ownership all live on the Yarah cloud. Using `flyctl`
> with your own credentials will land in the wrong Fly org and fail with
> `unauthorized`. Use `npx -y @yarahdev/cli compute …` instead.

Deploy a backend service. Two modes:
1. **Source mode** (`compute deploy [dir]`): you have a Dockerfile. CLI shells out to `flyctl deploy --remote-only --build-only` using a short-lived per-app deploy token minted by Yarah cloud. Build runs on Fly's remote builder; image is pushed to `registry.fly.io`. Cloud then launches the machine. **No local Docker daemon needed** — only `flyctl` on PATH.
2. **Image mode** (`compute deploy --image <url>`): deploy a pre-built image from any registry. **Nothing needed locally** beyond the Yarah CLI.

> Looking to deploy a **frontend** (static site / SPA / Next.js to Vercel)? Use
> `npx -y @yarahdev/cli deployments deploy` instead — see
...
**Anti-pattern: `flyctl deploy` directly from your laptop with your own credentials.** Returns 401 — the Fly account is Yarah's, not yours. The CLI invokes flyctl for you with the *cloud-minted* per-app token, which is the only token that works.
...
npx -y @yarahdev/cli compute deploy <dir> --name <name> [options]

# Image mode — deploy pre-built image (nothing needed locally).
npx -y @yarahdev/cli compute deploy --image <url> --name <name> [options]
```
...
npx -y @yarahdev/cli compute deploy . --name my-api --port 8000

# Off-the-shelf image
npx -y @yarahdev/cli compute deploy --image nginx:alpine --name proxy --port 80

# Pre-built image from GHCR
npx -y @yarahdev/cli compute deploy \
...
npx -y @yarahdev/cli compute deploy ./worker \
  --name batch \
  --port 8080 \
...
npx -y @yarahdev/cli compute update <id> \
...
npx -y @yarahdev/cli compute update <id> --env '{"NODE_ENV":"production","DATABASE_URL":"..."}'
```
...
# Deploy:
$ npx -y @yarahdev/cli compute deploy . --name my-bot --port 8080
...
2. CLI requests a per-app deploy token from the cloud — a Fly macaroon attenuated with `IfPresent { ifs: [Apps[<thisAppOnly>: rwcdC], FeatureSet[builder, wg]], else: deny }` and a 20-min ValidityWindow. The org-wide Fly token never leaves Yarah's servers.
...
The Yarah skill knows these patterns; ask the agent and it'll write one.
...
Yarah does **not** maintain a hardcoded allow-list — Fly.io is the source of truth for which sizes actually exist. If you pass an unsupported combination (e.g. `performance-32x`), Fly returns a clean validation error at machine-create time.
...
CLI → OSS instance → Yarah cloud backend → Fly.io. The cloud:
...
| `Image pull error` (image mode) | Registry private without Yarah having creds | Push to a public image, or contact support to configure private registry creds |
| `Unauthorized` from registry (image mode) | Image is private and Yarah cloud doesn't have credentials | Make the image public, or use a public registry |
...
A: It's down. Yarah runs your containers on Fly's infrastructure — Fly's uptime is your uptime. For HA, you'd typically deploy multiple services in different regions (future feature).
...
A: After `flyctl` pushes your image, Fly asynchronously aliases the digest from the builder's namespace to your app's namespace. Until that propagates (usually < 8 s) the Machines API returns `400 MANIFEST_UNKNOWN` even though the digest is correct. The Yarah cloud silently retries 4 times with backoff `[2s, 4s, 8s]`, so you almost never see it. If retries exhaust, you get a structured `COMPUTE_IMAGE_NOT_AVAILABLE` 400 with `nextActions` telling you to re-run — re-runs are idempotent and typically succeed instantly because the alias has had time to propagate.

## Notes

- The user never needs to handle a Fly token. The Yarah cloud holds the org token; per deploy it mints an app-scoped, attenuated token (~20 min, `else: deny`) and the CLI exports it as `FLY_API_TOKEN` only for the duration of the flyctl subprocess.
