...
```bash
npx -y @yarahdev/cli db query <sql> [options]
```
...
```bash
# Basic query
npx -y @yarahdev/cli db query "SELECT * FROM posts LIMIT 10"

# Update rows
npx -y @yarahdev/cli db query "UPDATE posts SET status = 'published' WHERE id = 'post_123'"

# Insert rows
npx -y @yarahdev/cli db query "INSERT INTO posts (title, status) VALUES ('Hello', 'draft')"

# Delete rows
npx -y @yarahdev/cli db query "DELETE FROM posts WHERE archived = true"

# Inspect Postgres system catalog
npx -y @yarahdev/cli db query "SELECT table_schema, table_name FROM information_schema.tables WHERE table_schema = 'public'"

# Inspect Yarah-managed schema data
npx -y @yarahdev/cli db query "SELECT * FROM auth.users LIMIT 10"

# JSON output for scripting
npx -y @yarahdev/cli db query "SELECT count(*) FROM posts" --json
```
...
```bash
npx -y @yarahdev/cli db query "DO \$\$
DECLARE
...
- Yarah-managed/system schemas such as `auth`, `storage`, `realtime`, `payments`, `graphql`, `extensions`, `pg_catalog`, `information_schema`, or `system`: do not write or run DDL unless you are working on that specific feature module and its docs explicitly allow the operation.

Use `npx -y @yarahdev/cli db migrations new ...` and `npx -y @yarahdev/cli db migrations up ...` for schema changes on `public` application objects.
...
When writing SQL for Yarah, use these built-in references:
...
npx -y @yarahdev/cli db query "SELECT id, status FROM posts WHERE status IS NULL"

# Backfill missing row values
npx -y @yarahdev/cli db query "UPDATE posts SET status = 'draft' WHERE status IS NULL"
```
