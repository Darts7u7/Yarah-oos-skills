...
```bash
npx -y @yarahdev/cli db export [options]
```
...
```bash
# Full export to file
npx -y @yarahdev/cli db export --output backup.sql

# Schema only
npx -y @yarahdev/cli db export --no-data --output schema.sql

# Specific tables with functions
npx -y @yarahdev/cli db export --tables users,posts --include-functions --output partial.sql

# JSON format
npx -y @yarahdev/cli db export --format json --output backup.json

# Limited rows for development
npx -y @yarahdev/cli db export --row-limit 100 --output dev-data.sql
```
