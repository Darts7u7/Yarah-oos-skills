...
Use this reference when configuring pgvector with the Yarah CLI: vector
...
with `@yarahdev/sdk`, use the `yarah` app-integration skill's AI/RAG guidance after this backend
schema is in place.

## Migration Pattern

DDL belongs in a migration. Create a migration file with
`npx -y @yarahdev/cli db migrations new <name>`, put SQL like the example below in
that file, then apply it with `npx -y @yarahdev/cli db migrations up --all`.
