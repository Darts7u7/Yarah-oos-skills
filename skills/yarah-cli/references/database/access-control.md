# Database Access Control for Yarah
...
Yarah uses three built-in PostgreSQL roles:
...
Raw SQL from `db query` and migration files runs as `project_admin`. This role can manage and own objects in `public`; access to Yarah-managed schemas is restricted.
...
- Yarah migrations already run against `public`; schema-qualified references keep helper functions explicit.
...
Yarah grants broad DML privileges on `public` tables to `anon` and
