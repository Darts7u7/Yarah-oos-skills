...
```bash
npx -y @yarahdev/cli db migrations list
npx -y @yarahdev/cli db migrations fetch
npx -y @yarahdev/cli db migrations new <migration-name>
npx -y @yarahdev/cli db migrations up <migration-file-name-or-version>
npx -y @yarahdev/cli db migrations up --to <migration-file-name-or-version>
npx -y @yarahdev/cli db migrations up --all
```
...
npx -y @yarahdev/cli db migrations new create-posts

# Apply by exact filename
npx -y @yarahdev/cli db migrations up 20260418091500_create-posts.sql

# Apply by version
npx -y @yarahdev/cli db migrations up 20260418091500

# Apply all pending migrations through a target
npx -y @yarahdev/cli db migrations up --to 20260418110000

# Apply all pending migrations
npx -y @yarahdev/cli db migrations up --all

# JSON output
npx -y @yarahdev/cli db migrations list --json
```
...
1. **Use migrations for schema changes**
   ...
   - `project_admin` can manage and own objects in `public`, but access to Yarah-managed schemas is restricted.
...
| Mistake | Solution |
|---------|----------|
| Naming files manually with underscores or spaces | Use `npx -y @yarahdev/cli db migrations new <migration-name>` |
| Reaching for `db query` to create or alter schema | Use migration files for schema changes; reserve `db query` for row changes |
| Trying to alter Yarah-managed tables like app-owned tables | Keep generic schema, RLS, trigger, function, and grant changes on `public` application objects; use feature-specific docs for managed module hooks or RLS |
...
```text
1. Check remote history             → npx -y @yarahdev/cli db migrations list
2. Sync applied files when useful   → npx -y @yarahdev/cli db migrations fetch
3. Create the next migration file   → npx -y @yarahdev/cli db migrations new <migration-name>
4. Edit the SQL file                → migrations/<version>_<migration-name>.sql
5. Apply the migration              → npx -y @yarahdev/cli db migrations up <filename> or --all
