...
```bash
npx -y @yarahdev/cli db import <file> [options]
```
...
```bash
# Import SQL file
npx -y @yarahdev/cli db import backup.sql

# Import with table truncation
npx -y @yarahdev/cli db import backup.sql --truncate
```
...
- The file must be a valid SQL file (e.g., from `npx -y @yarahdev/cli db export`).
