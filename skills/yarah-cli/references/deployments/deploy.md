# npx -y @yarahdev/cli deployments deploy — frontend hosting (Vercel)

Deploy a frontend project (static site / SPA / Next.js / etc.) to Yarah
hosting (via Vercel) from its source directory.

> Looking to deploy a **backend** Docker container (API, worker)? Use
> `npx -y @yarahdev/cli compute deploy` instead — see
> [compute-deploy.md](../compute-deploy.md).

## Syntax

```bash
npx -y @yarahdev/cli deployments deploy [source-directory] [options]
```
...
Frontend apps need env vars (API URL, anon key, etc.) to connect to Yarah. Deploying without them produces a broken app. There are two ways to provide them:

**Option A — Persistent env vars (recommended for repeated deploys):**

```bash
# Check what's already configured
npx -y @yarahdev/cli deployments env list

# Set env vars — these persist across all future deployments
npx -y @yarahdev/cli deployments env set VITE_YARAH_URL https://my-app.us-east.apps.yarah.dev
npx -y @yarahdev/cli deployments env set VITE_YARAH_ANON_KEY ik_xxx

# Deploy the project source — persistent vars are applied automatically
npx -y @yarahdev/cli deployments deploy .
```

**Option B — Inline `--env` flag (one-off or override):**

```bash
npx -y @yarahdev/cli deployments deploy . --env '{"VITE_YARAH_URL": "https://my-app.us-east.apps.yarah.dev", "VITE_YARAH_ANON_KEY": "ik_xxx"}'
```
...
npx -y @yarahdev/cli deployments deploy . --env '{"VITE_API_URL": "https://my-app.us-east.apps.yarah.dev", "VITE_ANON_KEY": "ik_xxx"}'

# JSON output
npx -y @yarahdev/cli deployments deploy . --json
```
...
**CRITICAL: Always verify local build succeeds before deploying to Yarah.**
...
# - Vite: VITE_YARAH_URL
# - Next.js: NEXT_PUBLIC_YARAH_URL
# - CRA: REACT_APP_YARAH_URL
# - Astro: PUBLIC_YARAH_URL
cat > .env.production << 'EOF'
YARAH_URL=https://your-project.apps.yarah.dev
YARAH_ANON_KEY=your-anon-key
EOF
...
|-----------|--------|---------|
| Vite | `VITE_` | `VITE_YARAH_URL` |
| Next.js | `NEXT_PUBLIC_` | `NEXT_PUBLIC_YARAH_URL` |
| Create React App | `REACT_APP_` | `REACT_APP_YARAH_URL` |
| Astro | `PUBLIC_` | `PUBLIC_YARAH_URL` |
| SvelteKit | `PUBLIC_` | `PUBLIC_YARAH_URL` |
...
Alternatively, pass env vars inline: `npx -y @yarahdev/cli deployments deploy . --env '{"VITE_YARAH_URL": "...", "VITE_YARAH_ANON_KEY": "..."}'`

### Check Deployment Status

Wait 30 seconds to 1 minute, then check status with `npx -y @yarahdev/cli deployments status <id>`.
...
1. **Deploy the project source directory**
   ...
   - Never include `node_modules`, `.git`, `.env`, or `.yarah` in the upload
   - Large assets should go to Yarah Storage, not the deployment
