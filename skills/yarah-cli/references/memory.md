...
Every Yarah project has built-in agent memory: a platform-managed store of durable facts, decisions, preferences, and references that survive across sessions. It exists to solve the forgetting problem - the next session (or another agent) should not have to rediscover what this one already learned.

## Commands

```bash
npx -y @yarahdev/cli memory list                  # title index of stored memories (cheap - no AI call)
npx -y @yarahdev/cli memory recall "<query>"      # semantic + keyword recall; --scope, --limit, --threshold
npx -y @yarahdev/cli memory remember "<content>"  # store a memory; --kind, --title, --scope, --source
npx -y @yarahdev/cli memory remember --file <f>   # extract durable memories from a transcript/notes file
```
...
```bash
npx -y @yarahdev/cli memory list
```
...
```bash
npx -y @yarahdev/cli memory recall "how are snippet tags filtered"
```
...
```bash
npx -y @yarahdev/cli memory remember \
