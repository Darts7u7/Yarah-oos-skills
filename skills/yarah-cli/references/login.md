# npx -y @yarahdev/cli login

Authenticate with the Yarah platform.

## Syntax

```bash
npx -y @yarahdev/cli login [options]
```
...
In sandboxed environments (the ChatGPT app, remote/SSH sessions, containers), the browser runs on the host but the CLI's `127.0.0.1` callback server is inside the sandbox, so the default flow can never complete (it waits until its callback timeout). Use the device flow instead (requires `@yarahdev/cli` ≥ 0.2): no callback and nothing to paste — the user approves a short code in their browser while the CLI polls.

**Run it as two steps.** `login --device` prints the link, then keeps running until the user approves — but most agent harnesses only return a command's output when the process exits, so a single blocking run shows you nothing to relay. Bound the first run to capture the link, relay it, then rerun to complete (the rerun resumes the SAME pending code from `~/.yarah/pending-device.json`):

```bash
# Step 1 — capture the verification link (process is killed after 15s; that's expected)
timeout 15 npx -y @yarahdev/cli login --device --json 2>&1 || true
```
...
To sign in, ask the user to open:
  ...
  https://yarah.dev/auth/device?user_code=BCDF-GHJK
...
npx -y @yarahdev/cli login --device --json
```

If the user already approved, step 2 completes immediately with the `--json` success object; otherwise it polls until they do. Codes expire after 15 minutes; both steps must run with the same `$HOME`. In an interactive terminal (a human at a shell), skip the two-step dance — just run `npx -y @yarahdev/cli login --device` and wait.
...
```bash
npx -y @yarahdev/cli login --user-api-key "$YARAH_USER_API_KEY"
```
...
```bash
npx -y @yarahdev/cli login --email
```

Prompts for email and password interactively. For non-interactive use (CI/CD), set environment variables:

```bash
YARAH_EMAIL=user@example.com YARAH_PASSWORD=secret npx -y @yarahdev/cli login --email
```

## Credential Storage

Credentials are saved to `~/.yarah/credentials.json` with restricted file permissions (0600). The shape depends on the login method:
...
npx -y @yarahdev/cli login --user-api-key "$YARAH_USER_API_KEY" --json
...
YARAH_EMAIL=$EMAIL YARAH_PASSWORD=$PASSWORD npx -y @yarahdev/cli login --email --json
```
