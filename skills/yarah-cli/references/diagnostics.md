...
```bash
npx -y @yarahdev/cli diagnose
npx -y @yarahdev/cli diagnose --ai "<issue description>"
npx -y @yarahdev/cli diagnose metrics --range 24h
npx -y @yarahdev/cli diagnose advisor --severity critical
npx -y @yarahdev/cli diagnose db --check bloat,slow-queries
npx -y @yarahdev/cli diagnose logs --limit 100
npx -y @yarahdev/cli diagnose incident
```
...
```bash
npx -y @yarahdev/cli logs function.logs --limit 50
npx -y @yarahdev/cli logs postgres.logs --limit 50
npx -y @yarahdev/cli logs yarah.logs --limit 50
npx -y @yarahdev/cli logs postgrest.logs --limit 50
```

| Source                 | Description                   |
| ---------------------- | ----------------------------- |
| `yarah.logs`        | Main backend logs             |
...
| Auth or API error                 | `logs yarah.logs`                                                                   |
| API returning 500 errors          | `logs yarah.logs`, `logs postgrest.logs`                                            |
