# npx -y @yarahdev/cli functions deploy
...
```bash
npx -y @yarahdev/cli functions deploy <slug> [options]
```
...
npx -y @yarahdev/cli functions deploy my-handler --file ./my-handler.ts

# Deploy with a display name and description
npx -y @yarahdev/cli functions deploy cleanup-expired --file ./handler.ts --name "Cleanup Expired" --description "Removes expired records"

# Update an existing function
npx -y @yarahdev/cli functions deploy payment-webhook --file ./webhooks/payment.ts
```
...
2. **Store credentials as secrets** — use `npx -y @yarahdev/cli secrets add` for API keys, base URLs, etc.
3. **Check available functions first** before invoking from frontend
   - Call `npx -y @yarahdev/cli functions list` to see existing functions
...
| Mistake | Solution |
|---------|----------|
| Invoking non-existent function | Check functions first with `npx -y @yarahdev/cli functions list`, create if needed |
...
2. Deploy                         → npx -y @yarahdev/cli functions deploy {slug} --file <path>
3. Check status                   → npx -y @yarahdev/cli functions list
4. Ensure secrets are set         → npx -y @yarahdev/cli secrets add YARAH_BASE_URL https://...
5. Invoke from frontend           → yarah.functions.invoke('{slug}', { body: {...} })
```
