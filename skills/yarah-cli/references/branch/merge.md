# npx -y @yarahdev/cli branch merge
...
```bash
npx -y @yarahdev/cli branch merge <name> [options]
```
...
1. Inspect parent's current state and branch's current state for the conflicted object (e.g. `npx -y @yarahdev/cli db tables` / `db policies`).
...
```bash
$ npx -y @yarahdev/cli branch merge feat-rls-fix --dry-run --save-sql /tmp/diff.sql
BEGIN;
…
COMMIT;
...
$ npx -y @yarahdev/cli branch merge feat-rls-fix
2 added, 1 modified, 0 conflict(s).
