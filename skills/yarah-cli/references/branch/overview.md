# Backend Branches — `npx -y @yarahdev/cli branch`
...
1. **Re-source your dev server's `.env`** — `YARAH_URL` / `YARAH_ANON_KEY` change with the switch.
...
Repoints `.yarah/project.json` at the branch (or back at the original parent). Refuses if the target branch isn't `ready`.

> **Critical:** the dev server's `.env` is **not** updated by `switch`. The SDK reads `YARAH_URL` / `YARAH_ANON_KEY` from `.env`, so without re-sourcing, the SDK silently keeps hitting the previous backend. This is the #1 source of "I switched but my changes aren't showing up."
...
The first hop off the parent backs up `.yarah/project.json` to `.yarah/project.parent.json`. Subsequent branch ↔ branch switches don't touch the backup — `--parent` always returns to the original.
