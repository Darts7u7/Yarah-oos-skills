...
> ⚠️ **Private beta.** The Apify web scraper is rolling out to early-access projects, and is **cloud-only**. Self-hosted backends don't expose the `/webscraper/apify/*` endpoints, so connecting and scraping through Yarah won't work there. If `yarah webscraper apify connect` fails with `HTTP 404`, this project doesn't have it enabled yet; wait for the rollout or ask the Yarah team to enable it.

## 1. Connect (one-time)

```bash
npx -y @yarahdev/cli webscraper apify connect
```

Opens the Apify OAuth flow and stores a refreshable token in Yarah. After a successful connect the CLI automatically runs the auth bridge (step 2) so the local agent is immediately usable.

## 2. Auth bridge (per machine)

```bash
npx -y @yarahdev/cli webscraper apify login
```

Fetches the Yarah-managed token, installs the Apify CLI via npm if it is missing, runs `apify login --token <token>` (no browser), and installs Apify's official agent skills (including `apify-ultimate-scraper`, the scraping playbook used in step 3). Verify with `apify info`.

**Hard rule:** never run plain `apify login` (browser OAuth). On any Apify `401` or "not logged in" error, re-run `login`; Yarah re-fetches a fresh token. Do not fall back to browser-based login under any circumstance.

For the `apify-sdk-integration` path (app code using the `apify-client` package), the client reads `APIFY_TOKEN` from the environment instead of the CLI login. Get a fresh token the same way and export it (`export APIFY_TOKEN=<token>`); never hardcode or commit it; it is short-lived and managed by Yarah.
...
```
GET  <YARAH_BASE_URL>/api/webscraper/apify/token
...
- **Edge function:** `YARAH_BASE_URL` and `API_KEY` (the project admin key) are injected automatically. Read them with `Deno.env.get('YARAH_BASE_URL')` / `Deno.env.get('API_KEY')` and use `API_KEY` as the bearer. Nothing to configure.
- **Fly compute:** nothing is auto-injected. Set both yourself at deploy time (`compute deploy --env` / `--env-file`): the base URL, and the project admin `ik_…` key. Give the admin key a distinct name (for example `YARAH_ADMIN_KEY`) so it does not collide with any `API_KEY` your app already uses for something else, and read that name in the handler. A missing or wrong value here surfaces as a malformed request or a `401` from the token endpoint.

Use that `accessToken` for Apify API calls. The token is short-lived, so **do not cache it for the whole run**: a short edge function fetches it once at the start (it finishes well before expiry); a long compute job fetches it per batch, or re-fetches and retries on a `401`. Yarah always returns a freshly refreshed token, so no personal Apify API key needs to be stored.
