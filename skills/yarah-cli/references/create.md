# npx -y @yarahdev/cli create

Create a new Yarah project.

## Syntax

```bash
npx -y @yarahdev/cli create [options]
```
...
```bash
npx -y @yarahdev/cli create --json --name my-app --org-id org_123 --region us-east --template react
```
...
1. Creates the project via the Yarah Platform API
...
5. Installs Yarah Agent Skills via `npx skills add Darts7u7/Yarah-oos-skills`
6. Creates `.yarah/project.json` in the current directory
...
npx -y @yarahdev/cli create --json --name blog-app --org-id org_abc --region us-east --template react

# Create with empty template (no frontend scaffolding)
npx -y @yarahdev/cli create --json --name api-only --org-id org_abc --region eu-central --template empty
```

## Notes

- Requires authentication (`npx -y @yarahdev/cli login` first).
- Creates `.yarah/project.json` which links the directory to the project.
- Agent skills are auto-installed into `.agents/skills/yarah/`.
