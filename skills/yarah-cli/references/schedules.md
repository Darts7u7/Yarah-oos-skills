...
Use `npx -y @yarahdev/cli schedules` to create and manage cron-style backend jobs.

## Commands

- `npx -y @yarahdev/cli schedules list` - list all scheduled tasks, including ID, name, cron, URL, method, active state, and next run.
- `npx -y @yarahdev/cli schedules get <id>` - get schedule details.
- `npx -y @yarahdev/cli schedules create --name --cron --url --method [--headers <json>] [--body <json>]` - create a scheduled job.
- `npx -y @yarahdev/cli schedules update <id> [--name] [--cron] [--url] [--method] [--headers] [--body] [--active]` - update a scheduled job.
- `npx -y @yarahdev/cli schedules delete <id>` - delete a scheduled job.
- `npx -y @yarahdev/cli schedules logs <id> [--limit] [--offset]` - view execution logs.
...
npx -y @yarahdev/cli schedules create \
  --name "Cleanup Expired" \
  --cron "*/5 * * * *" \
  --url "https://my-app.us-east.apps.yarah.dev/functions/cleanup" \
  --method POST \
...
npx -y @yarahdev/cli schedules create \
  --name "Health Probe" \
  --cron "30 seconds" \
  --url "https://my-app.us-east.apps.yarah.dev/functions/probe" \
  --method GET

# Check execution history
npx -y @yarahdev/cli schedules logs <id>
```

## Cron Expression Format

Yarah accepts two cron formats:
...
1. Create secrets if needed with `npx -y @yarahdev/cli secrets add KEY VALUE`.
2. Create or verify the target function with `npx -y @yarahdev/cli functions list`.
3. Create the schedule with `npx -y @yarahdev/cli schedules create`.
4. Verify the schedule is active with `npx -y @yarahdev/cli schedules get <id>`.
5. Monitor execution logs with `npx -y @yarahdev/cli schedules logs <id>`.

## Best Practices

- Pick the right cron format for the cadence: 5-field cron for wall-clock cadence; interval syntax for sub-minute cadence.
- Store sensitive values as Yarah secrets and reference them from headers.
- Target Yarah functions for serverless scheduled tasks using `https://your-project.region.apps.yarah.dev/functions/{slug}`.
