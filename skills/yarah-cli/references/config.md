...
```bash
npx -y @yarahdev/cli config export [--out yarah.toml] [--force]
npx -y @yarahdev/cli config plan   [--file yarah.toml]
npx -y @yarahdev/cli config apply  [--file yarah.toml] [--dry-run] [--auto-approve]
```

## File location

`yarah.toml` lives at the project root, alongside `package.json` and `.yarah/project.json`. Safe to commit to git.
...
```json
{
  "written": "/abs/path/to/yarah.toml",
  "config": {
    "auth": {
...
- `npx -y @yarahdev/cli metadata` — read-only view of all backend config slices
- **yarah** app-integration skill `auth/sdk-integration.md` — how SDK code reads auth config at runtime
