...
```bash
npx -y @yarahdev/cli diagnose advisor [--severity critical|warning|info] [--category security|performance|health] [--limit <n>] [--json]
```
...
- **Not available when linked via `--api-key`.** Requires `yarah login` (Platform auth).
...
# Security focus
npx -y @yarahdev/cli diagnose advisor --category security

# Performance focus (often pairs with db-health to verify)
npx -y @yarahdev/cli diagnose advisor --category performance
npx -y @yarahdev/cli diagnose db --check slow-queries,index-usage
