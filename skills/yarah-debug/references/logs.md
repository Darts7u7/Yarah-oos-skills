...
```bash
npx -y @yarahdev/cli logs <source> [--limit <n>]
```
...
|--------|------------------|----------------|
| `yarah.logs` | Main backend (auth, API gateway, realtime, edge function dispatcher, deploy controller) | Auth/OAuth errors, realtime WS errors, generic 5xx, signup/login failures |
...
```bash
npx -y @yarahdev/cli diagnose logs [--source <name>] [--limit <n>]
```
...
For request-correlated symptoms (single failing URL), look for the request line in `postgREST.logs` (REST calls) or `yarah.logs` (auth/realtime/function dispatch) — both include the URL path.
...
npx -y @yarahdev/cli logs postgREST.logs --limit 100

# If postgREST log shows "SQL error", drop into postgres
npx -y @yarahdev/cli logs postgres.logs --limit 100
```
