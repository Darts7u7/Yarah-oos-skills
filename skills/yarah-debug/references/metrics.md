...
```bash
npx -y @yarahdev/cli diagnose metrics [--range 1h|6h|24h|7d] [--metrics <list>]
```
...
npx -y @yarahdev/cli diagnose metrics --range 6h

# 2. If CPU/memory spiked at a timestamp, line it up with errors
npx -y @yarahdev/cli diagnose logs --limit 200

# 3. If DB is the bottleneck (Postgres-heavy CPU patterns)
npx -y @yarahdev/cli diagnose db
```
