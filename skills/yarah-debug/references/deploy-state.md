...
```bash
# Frontend (Vercel)
npx -y @yarahdev/cli deployments list
npx -y @yarahdev/cli deployments status <id> [--sync] [--json]

# Edge function deploys
npx -y @yarahdev/cli logs function-deploy.logs [--limit <n>]
npx -y @yarahdev/cli functions list
```
...
1. `npx -y @yarahdev/cli logs function-deploy.logs --limit 50` — find the build/push error
2. `npx -y @yarahdev/cli functions list` — confirm the function did or didn't make it into the active list
3. Re-run `npx -y @yarahdev/cli functions deploy <slug>` if needed and capture stdout for the explicit error
...
- **Doesn't include compute service (Fly) deploy errors.** For compute services use `npx -y @yarahdev/cli compute events <id>` (machine lifecycle); container stdout/stderr is not yet exposed.
...
npx -y @yarahdev/cli deployments status <id> --json
...
npx -y @yarahdev/cli functions deploy my-handler

# 2. Backend-side deploy log
npx -y @yarahdev/cli logs function-deploy.logs --limit 50
