...
Use OpenRouter as the model gateway for new Yarah AI features. Yarah
provides the project-level OpenRouter key; app code should call OpenRouter from
server-side code and use Yarah for data, storage, auth, and RLS.
...
```bash
npx -y @yarahdev/cli ai setup
```

This fetches the active OpenRouter key from the linked Yarah backend and
...
unavailable, ask the user to upgrade `@yarahdev/cli` or copy the key from the
Yarah dashboard Model Gateway.
...
| [image-generation.md](image-generation.md) | Generate or edit images, then upload outputs to Yarah Storage |
...
| [embeddings-and-rag.md](embeddings-and-rag.md) | Generate embeddings, store/search vectors in Yarah pgvector, and build RAG |
| [models-list.md](models-list.md) | Discover model IDs, modalities, pricing, limits, and dimensions |

## Yarah Patterns

1. Run `npx -y @yarahdev/cli ai setup` before writing OpenRouter code.
2. Store generated files in Yarah Storage; save both `url` and `key` in the
   database.
3. Store embeddings in Postgres `vector(N)` columns; `N` must match the model's
   output dimension.
4. Use `@yarahdev/sdk` for database, auth, storage, realtime, and payments.
...
| Using `yarah.ai` for new features | Use OpenRouter APIs with the project OpenRouter key |
| Saving generated base64/media directly in Postgres | Upload to Yarah Storage and save `url` plus `key` |
