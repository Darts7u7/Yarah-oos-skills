...
then store durable outputs in Yarah Storage.
...
Start with [overview.md](overview.md): run `npx -y @yarahdev/cli ai setup` and keep
...
permanent asset store. Upload the generated file to Yarah Storage and save
...
await yarah.database.from('generated_images').insert([{
...
3. Persist generated images in Yarah Storage before storing database rows.
