...
Yarah for job tracking, scheduled polling, Storage, and frontend status.
...
5. When a job completes, download the video server-side with
   `OPENROUTER_API_KEY`, upload it to Yarah Storage, and save `video_url` and
   `video_key`.
6. Let the frontend read `video_jobs` from Yarah or subscribe via Realtime.
...
3. Store generated videos in Yarah Storage; save both `url` and `key` in the
   database.
