...
classification, and structured text. Use Yarah for sessions, messages,
...
Start with [overview.md](overview.md): run `npx -y @yarahdev/cli ai setup`, keep
`OPENROUTER_API_KEY` server-side, and initialize the OpenAI SDK with
`baseURL: 'https://openrouter.ai/api/v1'`.

## Store a Response

For a chatbot app, keep session/message rows in Yarah and protect them with
...
never trust a browser-supplied `user_id`. Yarah database inserts use array
format.
...
async function sendChatMessage(sessionId: string, content: string) {
  const { data: userData, error: userError } = await yarah.auth.getCurrentUser()
  if (userError || !userData?.user?.id) {
    throw userError ?? new Error('Authentication required')
  }

  const history = await yarah.database
    .from('chat_messages')
    .select('role, content')
  ...
  const { error: userInsertError } = await yarah.database
    .from('chat_messages')
    .insert([{
  ...
  const { data, error: assistantInsertError } = await yarah.database
    .from('chat_messages')
    .insert([{
      session_id: sessionId,
      user_id: userData.user.id,
...
1. Store chat sessions and messages in Yarah, not local browser state, when
...
5. Use Yarah Realtime to fan out newly inserted assistant messages to other
...
| Keeping chatbot history only in React state | Store sessions/messages in Yarah and secure them with RLS |
...
| Treating OpenAI SDK errors like Yarah `{ data, error }` results | OpenAI SDK calls throw; catch exceptions separately |
