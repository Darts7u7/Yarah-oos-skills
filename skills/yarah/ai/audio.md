...
wire audio features into Yarah Storage, database rows, and server-side app
flows.
...
Name the Yarah guide `audio.md` because it covers both STT and TTS.
Use "transcription" only for STT-specific tables, jobs, or UI.

## Speech-to-Text

For STT, upload the original audio file to Yarah Storage first, then
...
as audio bytes. Upload the result to Yarah Storage and save:
...
2. Store audio files in Yarah Storage and database rows with `url` plus `key`.
