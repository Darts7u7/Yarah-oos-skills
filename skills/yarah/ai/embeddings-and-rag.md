...
Yarah Postgres with pgvector. Yarah remains the database/vector store;
...
- [Yarah CLI pgvector setup](../../yarah-cli/references/database/vector.md)
...
Use Yarah for the durable content row and vector. Keep one embedding model
per vector column.
...
async function storeDocument(content: string) {
  ...
  const { data, error } = await yarah.database.from('documents').insert([{
    content,
...
async function askQuestion(sessionId: string, question: string) {
  ...
  const { data: documents, error: searchError } = await yarah.database.rpc('match_documents', {
  ...
  const { error: insertError } = await yarah.database.from('chat_messages').insert([
...
1. Design the Yarah tables first: source documents, vector column dimension,
