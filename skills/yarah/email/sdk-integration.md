...
Send custom transactional HTML emails via `yarah.emails.send`. Routes through AWS SES under per-project tenant identities, with automatic List-Unsubscribe headers and unsubscribe filtering.

> **🛑 No SMTP, no third-party packages.** `yarah.emails.send()` works on every paid project — no `nodemailer` / `resend` / `sendgrid` / `mailgun` / `postmark`, no `SMTP_HOST`, no API keys. The platform manages the SES sender. Custom sender domain → dashboard, not `package.json`.
>
> **Scope.** This module sends **custom** transactional emails (welcome, receipt, newsletter, alerts). For auth flows (signup verification, password reset, invites), use `yarah.auth.*` — those ship on **every plan**, also no SMTP.
...
Ensure your `.env` is configured with your Yarah URL and anon key. Get the anon key with `npx -y @yarahdev/cli secrets get ANON_KEY`. See the main [SKILL.md](../SKILL.md) for framework-specific variable names.
...
```javascript
const { data, error } = await yarah.emails.send({
  to: 'user@example.com',
...
- **`from` is a display name only.** The actual sending address is always `noreply@<appkey>.send.yarah.dev` (your project's SES tenant subdomain). You cannot send from `hello@yourdomain.com`.
...
```bash
curl -X POST "$YARAH_URL/api/email/send-raw" \
  -H "Authorization: Bearer $YARAH_ANON_KEY" \
...
|---|---|---|
| `On-demand email service is only available for paid plans` | Free tier project | Upgrade billing in the Yarah dashboard |
...
| `Invalid ARN provided` | Backend infra config issue (missing `AWS_ACCOUNT_ID` env on the cloud backend) | Not a client bug — file with Yarah support |
...
- **Installing `nodemailer` / `resend` / `sendgrid` / `mailgun` / `postmark` or asking for SMTP credentials.** The built-in service is already wired in — `yarah.emails.send()` is all you need.
...
| Task | Code |
|------|------|
| Send to one recipient | `yarah.emails.send({ to, subject, html })` |
| Send to many | `yarah.emails.send({ to: [a, b], cc, bcc, subject, html })` |
