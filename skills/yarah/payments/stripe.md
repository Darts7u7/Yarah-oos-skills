...
Use this guide when writing app code for Stripe payments through `@yarahdev/sdk`. For CLI setup, catalog sync, webhook setup, and fulfillment migrations, use the `yarah-cli` payments references first.

## Mental Model

- Stripe uses Products and Prices. A subscription is still created through Checkout using recurring Prices.
- Yarah SDK creates Stripe Checkout Sessions and Billing Portal Sessions.
...
Checkout and portal creation use the caller's Yarah token. If the app exposes billing subjects such as teams or organizations, add RLS before wiring UI.
...
```typescript
const { data, error } =
  await yarah.payments.stripe.createCustomerPortalSession('test', {
    subject: { type: 'team', id: teamId },
...
Stripe webhooks are managed by Yarah when the backend has a public URL:

```bash
npx -y @yarahdev/cli payments stripe webhooks configure --environment test
```

Yarah's Stripe managed event set:
...
Subscription events do not carry the app `metadata` sent at checkout. Resolve the billing subject from the subscription metadata embedded in the event payload — Yarah stamps `yarah_subject_type` and `yarah_subject_id` at checkout, and Stripe snapshots it onto subscription-generated invoices as `parent.subscription_details.metadata`. Check `invoice.metadata` next, then fall back to `payments.customer_mappings` (the same order Yarah uses internally):
...
| Mistake | Fix |
|---------|-----|
| Calling payment methods at the root `payments` namespace | Use `yarah.payments.stripe.*` |
...
| Resolving subscription subjects only via `payments.customer_mappings` | Read `yarah_subject_*` from the event payload first; events from other types may not have created the mapping yet |
