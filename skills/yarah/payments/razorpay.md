...
Use this guide when writing app code for Razorpay payments through `@yarahdev/sdk`. For CLI setup, catalog sync, manual webhook instructions, and fulfillment migrations, use the `yarah-cli` payments references first.
...
- Razorpay Checkout runs inside the app; Yarah returns `checkoutOptions`, not a hosted redirect URL.
...
Razorpay runtime routes use the caller's Yarah token for authorization probes.
...
const checkout = new Razorpay({
  ...
  handler: async (response) => {
    const verified = await yarah.payments.razorpay.verifyOrder('test', {
...
const checkout = new Razorpay({
  ...data.checkoutOptions,
  handler: async (response) => {
    const verified = await yarah.payments.razorpay.verifySubscription('test', {
...
Razorpay webhook setup is manual. From the Yarah dashboard (Dashboard -> Payments -> Settings -> Webhooks), copy:

- Webhook URL, for example `/api/webhooks/razorpay/test`
- Webhook secret

Then create a webhook in the Razorpay Dashboard with that URL and secret, and select the events Yarah handles.

Razorpay can only deliver webhooks to a public HTTPS URL.

Events Yarah handles:
