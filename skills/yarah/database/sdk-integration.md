...
Use Yarah SDK to perform CRUD operations in your frontend application.
...
const { data, error } = await yarah.database.from('posts').select('id, title')

// All columns — reserve for single-record detail views; bound list reads with .limit()
const { data } = await yarah.database
  .from('posts')
  .select()
...
// With relationships
const { data } = await yarah.database.from('posts').select('*, comments(id, content)')
```
...
```javascript
const { data, error } = await yarah.database.rpc('get_user_stats', { user_id: '123' })
```
...
const { error: insertError } = await yarah.database
  .schema('analytics')
  .from('events')
  .insert([{ name: 'signup' }])

const { data: rollup, error: rpcError } = await yarah.database
  .schema('analytics')
...
const { data, count } = await yarah.database
  .from('posts')
...
- **Never poll with unbounded selects.** A dashboard or hook that re-runs `select()` (all columns, no limit) on an interval re-downloads the entire table every cycle. A table with a few hundred rows of JSONB polled every 30 seconds costs ~1 GB/day with a single viewer. To watch for changes, subscribe with [realtime](../realtime/sdk-integration.md) instead of polling — realtime is channel pub/sub, so pair the subscription with a database trigger that calls `realtime.publish(...)` on writes (backend setup: [yarah-cli realtime reference](../../yarah-cli/references/realtime.md)); table changes do not stream automatically. If you must poll, poll something cheap — `select('updated_at', { count: 'exact' }).order('updated_at', { ascending: false }).limit(1)` — and fetch full rows only when the count or newest `updated_at` moves: the count catches inserts and deletes, the timestamp catches edits (keep a `system.update_updated_at()` trigger on the table so edits advance it).
...
## Yarah SQL References

When creating tables via `yarah db query` (CLI), use these built-in references:
...
-- Policies (see the yarah-cli database access-control reference for advanced patterns)
...
> For access-control best practices (RLS, infinite recursion prevention, SECURITY DEFINER, performance), see the **yarah-cli** skill's [database/access-control.md](../../yarah-cli/references/database/access-control.md) reference.
...
1. **Generate TypeScript interfaces for every table schema**
   - Use `yarah db tables` and `yarah db query` (CLI) to inspect the table schema
...
```typescript
// After checking table schema via `yarah db tables`
...
```
1. Check table schema     → yarah db tables / yarah db query
