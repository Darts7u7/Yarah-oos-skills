# Storage Row Level Security (RLS) for Yarah

## Overview

Yarah governs `storage.objects` with PostgreSQL Row Level Security, not with app-side `WHERE uploaded_by = $1` filters. A signed-in caller runs as `authenticated` with their JWT `sub` available via `auth.jwt() ->> 'sub'`. Admin/API-key callers bypass RLS for admin workflows.
...
### Path helpers shipped with Yarah
...
Yarah exposes two write surfaces against the same `storage.objects` table:
...
- [Database access-control CLI reference](../../yarah-cli/references/database/access-control.md) — patterns for application tables, helper-function tricks, infinite-recursion gotchas
