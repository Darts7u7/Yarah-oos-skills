...
Use `createBrowserClient()` for authenticated browser uploads. It reads the browser-readable `yarah_access_token` cookie and refreshes through `/api/auth/refresh`; the refresh token remains httpOnly.
...
const { data: post } = await yarah.database
  .from('posts')
  .select('image_key')
  .eq('id', 'post-123')
  .single()
...
// Download using key
const { data: blob, error } = await yarah.storage
  .from('images')
...
- In Next.js / SSR apps, direct browser uploads should use `createBrowserClient()` from `@yarahdev/sdk/ssr` so Storage RLS sees the signed-in user
...
1. **Verify bucket exists before uploading**
   - Check available buckets via CLI: `yarah storage buckets`
...
```
1. Check available buckets → yarah storage buckets
