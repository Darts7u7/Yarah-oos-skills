...
User authentication, registration, and session management via `yarah.auth`.

> **Package choice**: Use `@yarahdev/sdk` directly for authentication flows. Build auth UI components with the SDK methods documented below.

> **Auth email delivery is built in.** Signup verification, password reset, magic links, and invites ship on **every plan** (free included) through the Yarah platform. For custom transactional email, see [email/sdk-integration.md](../email/sdk-integration.md).
...
For Next.js, Remix, SvelteKit, Nuxt server routes, or any other SSR setup, use `@yarahdev/sdk/ssr` helpers. See [ssr-integration.md](ssr-integration.md) for the full pattern and minimal examples.
...
|-----|-----------|-------------------|
| OAuth provider callback | Yarah backend (`https://<project>.apps.yarah.dev/api/auth/oauth/<provider>/callback`) | Google Console, GitHub OAuth app, etc. |
| `redirectTo` | **Your app** (`https://yourapp.com/auth/callback`) | Passed in `signInWithOAuth()` |

`redirectTo` is where the user lands after auth. The backend appends `?yarah_code=<code>` to it. Set it to a page where your app initializes the SDK or handles the server callback.
...
Use `additionalParams` only for provider-specific optional hints. Do not pass server-owned OAuth fields such as `client_id`, `scope`, `redirect_uri`, `code_challenge`, `state`, or `response_type`; Yarah sets those server-side and ignores colliding client-provided keys.

The SDK constructor auto-detects `yarah_code` in the URL, exchanges it for a session, and cleans the URL. Initialize the SDK on the `redirectTo` page.
...
```javascript
const { data } = await yarah.auth.signInWithOAuth('google', {
...
| `exchangeOAuthCode(code, codeVerifier?)` | Exchange `yarah_code` for session. Auto-called in SPA; call manually in SSR |

## Sign Out

```javascript
const { error } = await yarah.auth.signOut()
```
...
For SSR apps, use `createRefreshAuthRouter()` / `refreshAuth()` from `@yarahdev/sdk/ssr` to refresh through your app route.

### Cold loads & external redirects

In SPA browser apps using the root `@yarahdev/sdk` client, the access token is stored in memory only. On a cold page load, `getCurrentUser()` starts with no in-memory access token, so the SDK rehydrates the session by calling the backend refresh endpoint with the httpOnly refresh cookie and the JS-readable `yarah_csrf_token` cookie/header flow. During that network round-trip, `user` is temporarily `null`.

In SSR browser clients created with `createBrowserClient()` from `@yarahdev/sdk/ssr`, the access token is read from the `yarah_access_token` cookie and refreshed through your app's `/api/auth/refresh` route.
...
useEffect(() => {
  ...
  yarah.realtime.on('status_changed', handleStatusChanged)
  return () => yarah.realtime.off('status_changed', handleStatusChanged)
}, [resourceId])
...
const { data } = await yarah.auth.getProfile('user-id')

// Update current user's profile
const { data } = await yarah.auth.setProfile({
  name: 'John',
  avatar_url: 'https://...',
  custom_field: 'value'
})
...
const { data, error } = await yarah.auth.verifyEmail({
  email: 'user@example.com',
  otp: '123456'
})
...
- `yarah_status`: `success` or `error`
- `yarah_type`: always `verify_email`
- `yarah_error`: present only on error

When `yarah_status=success`, show a confirmation message and ask the user to sign in with their email and password.
...
const { data } = await yarah.auth.exchangeResetPasswordToken({
  email: 'user@example.com',
  code: '123456'
})
...
- `yarah_status`: `ready` or `error`
- `yarah_type`: always `reset_password`
- `yarah_error`: present only on error

Only render the reset form when `yarah_status=ready` and `token` is present.
...
- **SSR apps should use SDK SSR helpers**: For Next.js and similar SSR frameworks, use `@yarahdev/sdk/ssr` for client creation, refresh routes, and auth cookies; use `@yarahdev/sdk/ssr/middleware` for Proxy/Middleware session updates. See [ssr-integration.md](ssr-integration.md)
...
1. **Always check auth config first** before implementing
   - Run `npx -y @yarahdev/cli metadata --json` to get auth config (`requireEmailVerification`, `verifyEmailMethod`, `resetPasswordMethod`, `oAuthProviders`, `allowedRedirectUrls`)
   - This tells you what features to implement
   - To change supported project config such as redirect URLs, verification flags, password policy, or auth SMTP settings, use `npx -y @yarahdev/cli config apply` — see the **yarah-cli** skill's Configuration section. OAuth providers and external app setup are dashboard/provider-managed.
...
5. **Use `@yarahdev/sdk/ssr` for SSR auth**
   ...
   - Keep `yarah_refresh_token` httpOnly and server-owned
   - Let `yarah_access_token` be browser-readable so Storage and Realtime can authenticate from Client Components
   ...
   - Add `/api/auth/refresh` with `createRefreshAuthRouter()` and use `updateSession()` from `@yarahdev/sdk/ssr/middleware` in Proxy/Middleware
...
| Handling SSR auth like a browser-only flow | Use `createAuthActions()` for auth mutations, `createBrowserClient()` only to consume an existing SSR session, and `@yarahdev/sdk/ssr/middleware` for Proxy/Middleware session updates |
...
if (data?.requireEmailVerification) {
  ...
  const { data: verifyData, error } = await yarah.auth.verifyEmail({ email, otp: userEnteredCode })
...
```
1. Get auth config           → npx -y @yarahdev/cli metadata --json
