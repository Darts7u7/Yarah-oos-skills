...
- Use `@yarahdev/sdk/ssr` as the standard SSR auth entrypoint for browser,
  server, refresh-route, and cookie helpers.
- Use `@yarahdev/sdk/ssr/middleware` for Proxy/Middleware `updateSession()`;
  this keeps the middleware bundle from pulling in the full SDK client.
- Let the SDK helpers manage the Yarah auth cookie names and expiration.
- Keep `yarah_refresh_token` httpOnly and server-owned.
- Allow `yarah_access_token` to be browser-readable so browser SDK calls and Realtime can authenticate.
...
|--------|------------|---------|
| `yarah_access_token` | `httpOnly: false` | Short-lived bearer token for Server Components, Client Components, Storage, and Realtime |
| `yarah_refresh_token` | `httpOnly: true` | Long-lived server-owned refresh credential |

Both cookies should expire at the JWT `exp`; the SDK helpers do this when tokens include `exp`.

`yarah_access_token` is intentionally readable by JavaScript so browser SDK calls, Storage, and Realtime can authenticate directly. Keep access-token TTL short. If a project requires fully httpOnly auth tokens, proxy Storage and Realtime calls through server-side routes instead of direct browser SDK calls.
...
```bash
NEXT_PUBLIC_YARAH_URL=https://your-project.apps.yarah.dev
NEXT_PUBLIC_YARAH_ANON_KEY=...
NEXT_PUBLIC_APP_URL=https://your-app.example
```

The SSR helpers use explicit `baseUrl` / `anonKey` when provided. Otherwise they read `NEXT_PUBLIC_YARAH_URL` / `NEXT_PUBLIC_YARAH_ANON_KEY` in both browser and server code. `NEXT_PUBLIC_APP_URL` is the public app origin used by OAuth redirect examples. Missing config throws a clear error.
...
files. Keep the full `@yarahdev/sdk/ssr` entrypoint for `createBrowserClient()`,
...
`createAuthActions()` wraps `createServerClient()` and writes or clears `yarah_access_token` / `yarah_refresh_token` using the cookie stores you pass. Keep refresh separate: `/api/auth/refresh` should still use `createRefreshAuthRouter()` or `refreshAuth()`.

## OAuth In Next.js

The browser SDK auto-detects `yarah_code` for SPA flows. In SSR apps, handle OAuth on the server so the refresh token lands in an httpOnly cookie.
...
Set `redirectTo` to your app URL. The backend appends `?yarah_code=<code>` and redirects there.
...
export async function GET(request: NextRequest) {
  ...
  const cookieStore = await cookies()
  const codeVerifier = cookieStore.get('yarah_code_verifier')?.value
  if (!codeVerifier) {
...
- Use `updateSession()` from `@yarahdev/sdk/ssr/middleware` in Proxy/Middleware to keep Server Components and browser cookies aligned without bundling the full SDK client.
...
| Mistake | Fix |
|---------|-----|
| Creating separate cookie names across routes | Let `@yarahdev/sdk/ssr` helpers manage the standard auth cookie names |
| Hiding the access token from Client Components | Keep `yarah_refresh_token` httpOnly and `yarah_access_token` browser-readable |
...
| Server Components reading stale cookies | Use `updateSession()` from `@yarahdev/sdk/ssr/middleware` in Proxy/Middleware before rendering |
